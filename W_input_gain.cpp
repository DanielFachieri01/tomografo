...
void W_input_gain() {
    ...
    for (const auto& [window, pressure, proportion, type] : conditions) {
        ...
        oss << "W_" << type << "_" << proportion << "_" << (100 - proportion)
    ...
    std::vector<std::tuple<int,int>> limits = {
        {0, 1}, {0, 1}, {0, 1}, {25000, 30000}, {25000, 30000}
    };
    
    std::vector<double> max = {4*4096,4*4096,2058,12*4096,12*4096};

    

    TFile* file = TFile::Open("W_no_gain.root");
    ...
    for (size_t nome_idx = 0; nome_idx < names.size(); ++nome_idx) {
        ...
        TH1F* pdf = new TH1F(("pdf_" + multi_names[nome_idx]).c_str(), "Gain PDF", 200, 0, 150);
        ...
        TH1F* spectra = new TH1F(("spectra_" + names[nome_idx]).c_str(), names[nome_idx].c_str(), 512, 6000,max[nome_idx]);

...
double gamma_W(TRandom3& rnd) {
    // Distribuição de energia dos fótons de W
    // Valores aproximados baseados em espectros típicos de raios-X
    // Fonte: https://xdb.lbl.gov/
    // Valores de energia em eV
    // K alpha 1, K alpha 2, K beta 1, K beta 2
    // As porcentagens são aproximadas e podem variar dependendo da fonte
    // de raios-X utilizada.

    double total = 188;
    double r = rnd.Uniform(0., 100.);
    if (r < 53.19) return 59318.2; //K alpha 1
    else if (r < 82.4) return 57981.7; //K alpha 2
    else if (r < 9414) return 67244.1; //K beta 1
    else return 69067; //K beta 2
}
...
double gamma_Energy(TRandom3& rnd, double emax) {
    double r = rnd.Uniform(0, 100);
    if (r < 40) return fundo(rnd, emax);
    else return gamma_W(rnd);
}
...
int main(int argc, char* argv[]) {
    ...
    TFile* file = TFile::Open("W_no_gain.root", "RECREATE");
    ...
    for (const auto& condition : conditions) {
        ...
        oss << "W_"<<tipo << "_" << int(proportion * 100) << "_" << 100 - int(proportion * 100)
        ...
        for (int idx = 0; idx < nevents; idx++) {
            ...
            // aqui eu to cortando as energias de fótons que vindos do cobre porque vou
            // desconsiderar tudo abaixo de 10keV no discriminador
            if (gamma_origin < 1.1) {

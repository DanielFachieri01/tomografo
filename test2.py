...
class Application:
    ...
    def conectar(self):
        try:
            ...
            self.multimeter.write(":SENSe:CURRent:DC:RANGe 1e-7") 
            self.multimeter.write(":SYSTem:ZCHeck OFF")
            #self.multimeter.write(":SOURce:VOltage:LEVel:Ramp:TIME 20")
    ...
    def botoes(self):
        ...
        self.lb_passo = Label(self.root, text='ΔV', bg=cor10)
        ...
        #Save data
        self.save_btn = Button(self.root, text="Save Data", relief="groove", command=self.salvar_como)
        self.save_btn.place(relx=posx_btn, rely=posy + 2*offset_y, relwidth=btnw, relheight=h)
        #Save Graph        
        self.graph_btn = Button(self.root, text="Save Graph", relief="groove", command=self.save_graph)
        self.graph_btn.place(relx=posx_btn, rely=posy + 3*offset_y, relwidth=btnw, relheight=h)
        #Exit
        self.exit_btn = Button(self.root, text="Exit", relief="groove", command=self.root.destroy)
        self.exit_btn.place(relx=posx_btn, rely=posy + 5*offset_y, relwidth=btnw, relheight=h)
        # Botão pra conectar
        self.bt_connect = Button(self.root, text="Connect", command=self.conectar, relief="groove")
        ...
        # Botão Limpar
        self.bt_clear = Button(self.root, text="Clear", command=self.limpar, relief="groove")
        self.bt_clear.place(relx=posx_btn, rely=posy + 4*offset_y, relwidth=btnw, relheight=h)

    def fundo(self):
        # Carregar a imagem

        self.bg_image = Image.open(r"C:\Users\Hepic\Desktop\6517B\Daniel\logo_simples.jpg")
    ...
    def iniciar_varredura(self):
        ##começar do zero
        self.multimeter.write(":SYSTem:ZCHeck OFF")
        self.iniciar_rampdown()


        # Ler valores das entradas
        self.v0_val = float(self.v0.get())
        self.v1_val = float(self.v1.get())
        self.dv = int(self.passo.get())

        self.x_values = np.arange(self.v0_val, self.v1_val+1, self.dv)

        # Configurações iniciais
        self.multimeter.write(":OUTput1:STATe ON")
        self.multimeter.write(":SOURce:VOLTage:MCONnect ON ")     # liga a alta tensão
        self.multimeter.write(":SOURce:VOLTage:RANGe MAXimum")    #ajusta o range da alta tensão
    ...
    def plot_varredura(self):
        if self.running and self.current_index < len(self.x_values):
            # Adiciona próximo ponto
            self.multimeter.write(f":SOURce:VOltage:LEVel:IMMediate:AMPLitude {self.x_values[self.current_index]}")
            self.dynamic_x.append(-1*float(self.multimeter.query(":SOUR:VOLT?"))) #salvando o valor de tensão que o keithley tá mandando
            self.dynamic_y.append(float(self.multimeter.query(":MEAS:CURR?").split(',')[0][:-4])*1e9) #salvando o valor de corrente que o keithley tá medindo
            self.current_index += 1

            # Atualiza gráfico
            self.ax.clear()
            self.ax.plot(self.dynamic_x , self.dynamic_y, color='Black', marker='o', markerfacecolor = "Red")
            self.ax.set_xlabel("ΔV GEM (V)")
            self.ax.set_ylabel("Corrente (nA)")
            self.ax.set_title("Caracterização")
            self.canvas.draw()

            # Chama novamente após 100ms
            self.root.after(50, self.plot_varredura)
        else:
            self.runnig = False
            self.multimeter.write(":SYSTem:ZCHeck ON")
            self.multimeter.write(":OUTput1:STATe OFF")
            self.multimeter.write()
    ...
    def iniciar_rampdown(self):
        
        tensao_inicial = float(self.multimeter.query(":SOUR:VOLT?").strip())
        self.rampdown_vals = list(np.linspace(tensao_inicial, 0, 100))  # até menor que 0
        self.rampdown_step()  

    def rampdown_step(self):
        if self.rampdown_vals:
            volt_value = self.rampdown_vals.pop(0)
            self.multimeter.write(f":SOUR:VOLT {volt_value}")
            self.root.after(100, self.rampdown_step)
        else:
            return
    
    def inicio(self):
        self.ramp_vals = list(np.linspace(0,self.v0_val,100))
        self.ramp_step()
    
    def ramp_step(self):
        if self.ramp_vals:
            self.multimeter.write(f":SOUR:VOLT {self.ramp_vals.pop(0)}")
            self.root.after(5,self.ramp_step)
        else:
            return 
    def limpar(self):
        self.iniciar_rampdown()
